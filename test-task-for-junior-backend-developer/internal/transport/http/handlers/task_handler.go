...
func (h *TaskHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	created, err := h.usecase.Create(r.Context(), taskusecase.CreateInput{
		...
		Status:      req.Status,
		Recurrence:  newRecurrenceDomain(req.Recurrence),
	})
	if err != nil {
...
func (h *TaskHandler) Generate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, http.StatusBadRequest, errors.New("invalid from_date format, expected YYYY-MM-DD"))
		return
	}
	toDate, err := time.Parse("2006-01-02", req.ToDate)
	if err != nil {
		writeError(w, http.StatusBadRequest, errors.New("invalid to_date format, expected YYYY-MM-DD"))
		return
	}

	tasks, err := h.usecase.Generate(r.Context(), id, taskusecase.GenerateInput{
		FromDate: fromDate,
		ToDate:   toDate,
	})
	if err != nil {
